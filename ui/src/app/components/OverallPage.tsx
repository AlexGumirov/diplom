...
function getDeltaMessage(latestDelta: DeltaInfo | null) {
  if (!latestDelta || latestDelta.delta === null) {
    return 'Нужно больше данных для дельта-анализа.'
  }
  if (latestDelta.status === 'improvement') {
    return `Состояние улучшилось на ${latestDelta.delta}.`
  }
  if (latestDelta.status === 'deterioration') {
    return `Состояние ухудшилось на ${Math.abs(latestDelta.delta)}.`
  }
  return `Состояние стабильно, изменение ${latestDelta.delta}.`
}

export function OverallPage({ records, latestDelta }: OverallPageProps) {
  const [showDynamic, setShowDynamic] = useState(false)

  const overallData = useMemo(
    () =>
      records
        .filter((record) => record.total_score !== null)
        .slice(0, 7)
        .reverse()
        .map((record) => ({
          ...
          score: record.total_score as number,
        })),
    [records]
  )

  const latestRecord = records.find((record) => record.total_score !== null) ?? null
  const currentScore = latestRecord?.total_score ?? null
  const currentDate = latestRecord?.date
  const isPositiveDynamic = latestDelta ? latestDelta.status !== 'deterioration' : true
        ...
        <div className="bg-card rounded-lg shadow-sm border border-border p-6">
          ...
          <div className="w-full" style={{ height: '400px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={overallData} margin={{ top: 20, right: 30, left: 0, bottom: 20 }}>
                ...
                />
                <Bar dataKey="score" fill="var(--primary)" radius={[8, 8, 0, 0]} name="Общее состояние" />
                {showDynamic && overallData.length > 1 && (
                  <Line
                    type="monotone"
                    dataKey="score"
